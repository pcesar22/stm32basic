...
static void write(char * text)
{
    ...
    {
        UartReady = RESET;
        if (HAL_UART_Transmit_DMA(&huart3, (uint8_t *)text, len) != HAL_OK)
        {
            console_ErrorHandler();
        }
        while (UartReady != SET);
...
void console_Init(void)
{
    ...
    if (HAL_UART_Receive_DMA(&huart3, (uint8_t *)aRxBuffer, 1) != HAL_OK)
    {
        console_ErrorHandler();
    }
}
...
{
    UartReady = RESET;
    if (HAL_UART_Transmit_DMA(&huart3, (uint8_t *)&c, 1) != HAL_OK)
    {
        console_ErrorHandler();
    }
    while (UartReady != SET);
...
static void processByte(char byte)
{
    ...
    default:
        ...
        UartReady = RESET;
        // Can't use "write" for a single byte cause of the '\0' crap
        if (HAL_UART_Transmit_DMA(&huart3, (uint8_t *)&byte, 1) != HAL_OK)
        {
            console_ErrorHandler();
        }
        while (UartReady != SET);
...
{
    // Receive byte
    uint8_t byte_received = huart3->pRxBuffPtr[0];
 ...
 * @retval None
 */
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart3)
{
    console_ErrorHandler();
}
